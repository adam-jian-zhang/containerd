...
stateDiagram-v2
    ...
    Running --> Running: Exec (add process)
    Running --> Running: Update (resources)
```
...
stateDiagram-v2
    ...
    Committed --> Active: Prepare (with parent)
    Active --> [*]: Remove
    View --> [*]: Remove
    Committed --> [*]: Remove
```
...
sequenceDiagram
    ...
    Client->>Content Service: Write(stream)
    Content Service->>Ingest: Create temp file
    loop Stream chunks
        Client->>Content Service: WriteContentRequest
        Content Service->>Ingest: Append data
    end
    Client->>Content Service: Commit (with digest)
    Content Service->>Content Service: Verify digest
    Content Service->>Blobs: Move to blobs/sha256/
    Content Service-->>Client: Success
```
...
```mermaid
graph LR
    Publisher[Event Publisher] -->|Publish| Exchange[Event Exchange]
    Exchange -->|Filter| Sub1[Subscriber 1]
    Exchange -->|Filter| Sub2[Subscriber 2]
    Exchange -->|Filter| Sub3[Subscriber 3]
```
...
sequenceDiagram
    ...
    Daemon->>Shim: Create (via TTRPC)
    Shim->>Runtime: runc create
    Runtime-->>Shim: Success
    Shim-->>Daemon: CreateTaskResponse
    
    Daemon->>Shim: Start
    Shim->>Runtime: runc start
    Runtime-->>Shim: Success
    Shim-->>Daemon: StartResponse
    
    Shim->>Daemon: Event (task started)
    
    Note over Shim,Runtime: Container running
    
    Runtime->>Shim: Process exit
    Shim->>Daemon: Event (task exited)
    
    Daemon->>Shim: Delete
    Shim->>Runtime: Cleanup
    Shim-->>Daemon: DeleteResponse
```
...
```mermaid
graph LR
    V1[API v1] -->|Deprecated| V2[API v2]
    V2 -->|Future| V3[API v3]
    
    V1 -.->|Supported| Current[Current Release]
    V2 -->|Active| Current
```
...
graph TB
    ...
    Client -->|File permissions| Socket
    Socket -->|Namespace context| Daemon
    
    Note[Socket permissions control access]
```
...
sequenceDiagram
    ...
    Containers-->>Client: Container created
    
    Client->>Snapshots: Prepare(snapshot)
    Snapshots-->>Client: Mounts
    
    Client->>Tasks: Create(task, mounts)
    Tasks-->>Client: Task created
    
    Client->>Tasks: Start(task)
    Tasks-->>Client: Task started
```
...
sequenceDiagram
    ...
    loop For each layer
        Client->>Content: Write(layer)
    end
    
    Client->>Images: Create(image)
    Images-->>Client: Image created
    
    Client->>Snapshots: Prepare(layers)
    Snapshots-->>Client: Snapshot ready
```
